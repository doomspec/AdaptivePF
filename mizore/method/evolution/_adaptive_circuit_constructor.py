...
from ._utilities import get_n_measurement_with_var_coeff, get_var_coeff_by_lambda, \
    get_n_measurement
...
class TimeEvolutionConstructor():
    ...
    def __init__(self, n_qubit, block_pool, hamiltonian, quality_cutoff, task_manager,
                 nM_cutoff):
        ...
        self.__np_hamil = get_sparse_operator(hamiltonian,
                                              n_qubits=self.n_qubit).todense()
    ...
    def construct_circuit(self, circuit: BlockCircuit):
        ...
        while (current_quality > self.quality_cutoff) or (n_measurement > self.nM_cutoff):
            ...
            current_quality = calc_quality(derivative, parameter_directions,
                                           hamil_state_vec)

            n_measurement = get_n_measurement(derivative, self.hamiltonian_weight, mat_A,
                                              vec_C, current_quality)

            if (current_quality <= self.quality_cutoff) and (
                    n_measurement <= self.nM_cutoff):
                break
            ...
            for block in current_pool:
                task_list.append(QualityEvalTask(None, None, None, None, None, block,
                                                 self.hamiltonian_weight))
            for task in task_list:
                self.task_manager.add_task_to_buffer(
                    task, task_series_id=task_series_id)
            self.task_manager.smart_flush(
                public_resource={"parameter_directions": parameter_directions,
                                 "mat_A": mat_A, "vec_C": vec_C,
                                 "current_state_vec": current_state_vec,
                                 "hamil_state_vec": hamil_state_vec})
            ...
            nM_list = [get_n_measurement_with_var_coeff(item[1], item[0]) for item in
                       quality_and_var_coeff_list]
            ...
            log_path = os.environ["adaptive_pf_output"] + "/construction.log"
            # create log file and directory if not exist
            if not os.path.exists(log_path):
                os.makedirs(os.path.dirname(log_path), exist_ok=True)
                with open(log_path, "w") as f:
                    pass
            with open(os.environ["adaptive_pf_output"] + "/construction.log", "a") as f:
...
class QualityEvalTask(Task):
    ...
    def __init__(self, parameter_directions, mat_A, vec_C, current_state_vec,
                 hamil_state_vec, block, hamiltonian_weight):
        Task.__init__(self)
        self.mat_A = mat_A
        self.vec_C = vec_C
    ...
    def run(self):
        ...
        for i in range(n_new_para):
            for j in range(n_old_para):
                up_right_mat_A[j][i] = vdot(self.parameter_directions[j],
                                            new_parameter_directions[i])
        ...
        new_parameter_directions = vstack(
            [self.parameter_directions, new_parameter_directions])
        quality = calc_quality(derivative, new_parameter_directions, self.hamil_state_vec)

        var_coeff = get_var_coeff_by_lambda(derivative, self.hamiltonian_weight,
                                            new_mat_A, new_vec_C)
