...
def run_adaptive_product_formula(energy_obj, quality_cutoff=0.3, n_circuit=30,
                                 delta_t=0.2, stepsize=2e-3, n_worker=40, nM_cutoff=5e8,
                                 save_root=None):
    if save_root is None:
        save_root = output_root + "/adaptive_evolution"
    ...
    with TaskManager(n_worker) as tm:
        evolver = TimeEvolutionEvolver(bc.n_qubit, hamil, quality_cutoff=quality_cutoff,
                                       stepsize=stepsize, nM_cutoff=nM_cutoff)
        constructor = TimeEvolutionConstructor(n_qubit, pool, hamil, quality_cutoff / 2,
                                               tm, nM_cutoff * 0.9)
        smart_evolver = QualityEnsuredEvolver(bc, constructor, evolver, para_dict={
            "project_name": "{}_{}".format(energy_obj.obj_info["system_name"],
                                           str(quality_cutoff)), "is_to_save": True,
            "n_circuit": n_circuit, "save_root": save_root, "delta_t": delta_t})
